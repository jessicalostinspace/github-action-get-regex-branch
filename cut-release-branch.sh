set -e

regexString=$1

# base64 -D <<<""
# echo
echo $(git branch -a | grep $regexString  | var=release-v; awk -F$var '{ print $2 }' )
git for-each-ref --shell --sort=-committerdate refs/heads/
# git for-each-ref --shell --sort=-committerdate refs/heads/$regexString

echo $(git for-each-ref --shell --sort=-committerdate refs/heads/)

# git checkout -b "release-$version"
# git push --set-upstream origin "release-$version"
# echo "Created 'release-$version' branch and pushed to remote"